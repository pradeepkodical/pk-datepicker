...
import { getDaysInYear, addDays, getWeek, format, differenceInSeconds } from 'date-fns';
...
function CanvasContainer(props) {
    ...
    const height = useMemo(() => Math.min(20000, drawItems.reduce((p, c) => p + c.getHeight(), 0)), [drawItems]);
    const width = useMemo(() => {
    ...
    return (jsx("div", Object.assign({ style: {
            ...
            width: '100%',
            minHeight,
        } }, { children: jsx("canvas", { width: width, height: height, ref: canvasRef }, void 0) }), void 0));
}
...
function getMonthsHeader(year, config) {
    ...
    for (var i = 0; i < 12; i++) {
        ...
        if (i > 10 && w < 45)
            w = 53;
        arr.push(TextDrawItem.create((1 + w) * config.cellSize, 0, config.cellSize, config.textColor || '#111', config.defaultBgColor || '#fff', format(startDate, 'MMM'), {}));
    }
    return arr;
}
...
function YearsCalendar(props) {
    ...
    const theConfig = useMemo(() => (Object.assign(Object.assign({}, defaultConfig$4), getConfig(config))), [config]);
...
function CanvasChart(props) {
    ...
    return (jsxs("div", Object.assign({ ref: ref, style: {
            display: 'flex',
            padding: '10px',
...
function createDrawItems$3(items, theConfig) {
    const dItems = [];
...
function StackedBarChart(props) {
    ...
    const theConfig = useMemo(() => (Object.assign(Object.assign({}, defaultConfig$3), getConfig(config))), [config]);
    const getDrawItems = useCallback((width) => createDrawItems$3(items, theConfig), [items, theConfig]);
    return (jsx(CanvasChart, { getDrawItems: getDrawItems, onClick: onClick, theConfig: theConfig, tooltip: tooltip }, void 0));
}
...
function AchievementsChart(props) {
    const { items, config, tooltip, onClick } = props;
    const theConfig = useMemo(() => (Object.assign(Object.assign({}, defaultConfig$2), getConfig(config))), [config]);
    const getDrawItems = useCallback((width) => createDrawItems$2(items, theConfig), [items, theConfig]);
    return (jsx(CanvasChart, { getDrawItems: getDrawItems, onClick: onClick, theConfig: theConfig, tooltip: tooltip }, void 0));
}
...
function AttendeesChart(props) {
    ...
    const theConfig = useMemo(() => (Object.assign(Object.assign({}, defaultConfig$1), getConfig(config))), [config]);
    const getDrawItems = useCallback((width) => createDrawItems$1(attendees, events, theConfig), [attendees, events, theConfig]);
    return (jsx(CanvasChart, { getDrawItems: getDrawItems, onClick: onClick, theConfig: theConfig, tooltip: tooltip }, void 0));
}
...
function createItem(item, top, minStartDate, maxEndDate, nameWidth, cWidth, bgColor, theConfig) {
    ...
    c.items.push(BoxDrawItem.create(0, top, nameWidth, theConfig.textColor, item.bgColor || bgColor, theConfig.selBgColor, theConfig.borderColor, item, item.name)
        .setHeight(theConfig.rowSize)
        .setTextAlign('left')
        .setFont(`${theConfig.fontSize}px Verdana`));
    const maxWidth = Math.max(cWidth - nameWidth - theConfig.rowSize, 2.2 * nameWidth);
    const left = maxWidth * (s / t);
    const width = Math.max(15, maxWidth * (e / t) - left);
    c.items.push(BoxDrawItem.create(left, top, width, theConfig.textColor, item.itemBgColor || theConfig.itemColor, theConfig.selBgColor, theConfig.borderColor, item, '')
        .setHeight(theConfig.itemSize)
        .moveBy(nameWidth, (theConfig.rowSize - theConfig.itemSize) / 2));
    return c;
}
function createDrawItems(items, theConfig, width) {
    const arr = [];
    ...
    const nameWidth = items.reduce((p, c) => Math.max(p, c.name.length * theConfig.fontSize * 0.7), 0);
    items.forEach((item, i) => {
        arr.push(createItem(item, i * theConfig.rowSize, minStartDate, maxEndDate, nameWidth, width, i % 2 === 0 ? theConfig.alternateBgColor : theConfig.defaultBgColor, theConfig));
    });
    const c = new ContainerDrawItem();
    c.items = arr;
    c.height = c.getHeight();
...
function BasicGantChart(props) {
    const { items, config, tooltip, onClick } = props;
    const theConfig = useMemo(() => (Object.assign(Object.assign({}, defaultConfig), getConfig(config))), [config]);
    const getDrawItems = useCallback((width) => createDrawItems(items, theConfig, width), [items, theConfig]);
    return (jsx(CanvasChart, { getDrawItems: getDrawItems, onClick: onClick, theConfig: theConfig, tooltip: tooltip }, void 0));
}

export { AchievementsChart, AttendeesChart, BasicGantChart, StackedBarChart, YearsCalendar };
