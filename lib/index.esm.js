...
function StackedBarChart(props) {
    ...
    return (jsx(CanvasChart, { drawItems: drawItems, onClick: onClick, theConfig: theConfig, tooltip: tooltip }, void 0));
}
...
function AchievementsChart(props) {
    ...
    const drawItems = useMemo(() => createDrawItems$2(items, theConfig), [
        items,
        theConfig,
    ]);
    return (jsx(CanvasChart, { drawItems: drawItems, onClick: onClick, theConfig: theConfig, tooltip: tooltip }, void 0));
...
function AttendeesChart(props) {
    ...
    const drawItems = useMemo(() => createDrawItems$1(attendees, events, theConfig), [attendees, events, theConfig]);
    return (jsx(CanvasChart, { drawItems: drawItems, onClick: onClick, theConfig: theConfig, tooltip: tooltip }, void 0));
}
...
function createItem(item, top, minStartDate, maxEndDate, nameWidth, cWidth, bgColor, theConfig) {
    ...
    c.items.push(BoxDrawItem.create(0, top, nameWidth, theConfig.textColor, bgColor, theConfig.selBgColor, theConfig.borderColor, item, item.name)
...
function createDrawItems(items, theConfig) {
    ...
    const nameWidth = items.reduce((p, c) => Math.max(p, c.name.length * 6), 0);
    items.forEach((item, i) => {
